...
import { DepartmentDetails } from '@/components/modals/department/department-details';
...
const OwnerAdmin = () => {
  const { departmentColumns, departmentData } = useDepartmentData();

  const [textFilter, setTextFilter] = useState('name');
  ...
  const filteredData = departmentData.filter((department) => {
    ...
    switch (textFilter) {
      ...
      case 'createdBy':
        return department.createdBy?.toLowerCase().includes(searchValue);
      case 'regDate':
        return department.regDate
          ?.toString()
          .toLowerCase()
  ...
  const filterItems: MenuProps['items'] = [
    {
      label: (
        <p className='text-xs my-1 w-32' onClick={() => setTextFilter('name')}>
          Department Name
        </p>
      ),
      key: '0',
    ...
    {
      label: (
        <p className='text-xs my-1' onClick={() => setTextFilter('createdBy')}>
          Registered By
        </p>
      ),
      key: '1',
